...
colonnes = ['ville',
            ...
            '4 pièces',
            '5 pièces et plus',
            'Loyer mensuel moyen des logements sociaux',
            'Résidences principales de 30 m² à 40 m²',
            'Résidences principales construites de 1919 à 1945',
            'Temps passé par des locataires dans leur logement',
            ...
            'Age moyen des logements sociaux',
            'Temps passé en moyenne dans un logement',
            'Résidences principales de 120 m² et plus',
            'Temps passé dans un logement à titre gratuit',
            'Evolution du loyer mensuel moyen sur un an',
            ...
            'Temps passé par des locataires HLM dans leur logement',
            'Logements sociaux loués',
            'Logements sociaux en cours de réhabilitation',
            'Temps passé par des propriétaires dans leur logement',
...
print('liens restants à traiter:\033[93m\033[1m',len(listeLiens),'\033[0m - liens réalisés: \033[92m\033[1m',len(tableau),'\033[0m. Total :\033[95m\033[1m', len(tableauLiens),'\033[0m')
...
def parse(lien):
    ...
    if req.status_code == 200:
            ...
            #print('*lien:',lien,'len(tables):',len(tables))
            if len(tables) > 0:
                for i in range(len(tables)):
                    for info in tables[i].findAll('tr')[1:]:
                        ...
                        if 'Evolution du loyer mensuel' in cle:
                        #     #print('ici !!', valeur)
                        #     #print(valeur.split('+')[1].replace(',','.').replace('%','').strip())
                            #print(valeur.split('+')[1].replace(',','.').replace('%','').strip())
                            try:
                                dico[cle] = float(valeur.split('+')[1].replace(',','.').replace('%','').strip())

                            except:
                                dico[cle] = valeur.replace(',','.').replace('%','').strip()
                        
                        elif "Locataires hébergés" in cle:
                            try:
                                dico["Locataires hébergés à titre gratuit"] = float(valeur.split()[0].replace(',','.').replace('%','').strip())
                            except:
                                dico["Locataires hébergés à titre gratuit"] = valeur.split()[0].replace(',','.').replace('%','').strip()
                        
                        elif "5 pièces" in cle:
                            try:
                                dico["5 pièces et plus"] = float(valeur.split()[0].replace(',','.').replace('%','').strip())
                            except:
                                dico["5 pièces et plus"] = valeur.split()[0].replace(',','.').replace('%','').strip()
                        else:
                            #print(valeur.split()[0].replace(',','.').replace('%','').strip())
                            try:
                                dico[cle] = float(valeur.split()[0].replace(',','.').replace('%','').strip())
                            except:
                                dico[cle] = valeur.split()[0].replace(',','.').replace('%','').strip()
